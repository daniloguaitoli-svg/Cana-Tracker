...
export function reais(v, casas = 2) {
  if (v == null || !Number.isFinite(v)) return "—";
  return `R$ ${(casas > 2 ? nf4 : nf2).format(v)}`;
}
...
// Quantas casas decimais cada unidade merece (preços em R$/litro e R$/kg de ATR
// só fazem sentido com 4 casas).
export function casasDaUnidade(unidade) {
  return unidade === "BRL_LITRO" ||
    unidade === "USD_GALAO" ||
    unidade === "BRL_KG_ATR" ||
    unidade === "BRL_KG"
    ? 4
...
// Rótulo da periodicidade, para explicar por que um preço "parado" é normal.
export const PERIODICIDADE = { diaria: "diário", semanal: "semanal", mensal: "mensal" };
